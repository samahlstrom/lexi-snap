...
**Optional:** Install auto-start (requires administrator):
```bash
python platform_integration/windows_menu.py --install
```

#### Method 2: Using Compiled Executable (Coming Soon)

Download `DictToAnki.exe` from releases and run it once to install the service.
...
2. **Highlight a word** - Select any word in any application

3. **Right-click**:
   - **Windows**: Choose "Send to Anki" from context menu
   - **macOS**: Services → Send to Anki (or use keyboard shortcut)

4. **Select deck** - Choose which Anki deck to add the card to

5. **Done!** - The card is created automatically with:
   - **Front**: The word
   - **Back**: Top 3 definitions with examples + your context sentence
...
### Windows: Context menu not appearing

**Solution:**
1. Make sure you ran the installation as administrator
2. Try reinstalling: `python platform_integration/windows_menu.py --install`
3. Restart Windows Explorer (Ctrl+Shift+Esc → Restart "Windows Explorer")
...
- **src/text_capture.py** - Cross-platform text capture using clipboard simulation
...
- **platform_integration/windows_menu.py** - Windows context menu registry integration
...
2. **Word Extraction**: Extracts the first word from selection
3. **API Call**: Fetches definitions from free dictionary API
4. **Deck Selection**: Shows GUI dialog with available Anki decks
5. **Card Creation**: Formats and creates card via AnkiConnect
6. **Notification**: Shows success/error notification
...
4. **Context Capture**: Uses only the highlighted text as context (no automatic expansion)
5. **macOS Setup**: Requires manual Automator workflow creation
